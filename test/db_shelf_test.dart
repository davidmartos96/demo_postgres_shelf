...
void main() {
            ...
            unawaited(
              db.runTx((c) async {
                final applicationName =
                    await _getCurrentPostgresApplicationName(c);
  ...
  test('unexpected error should abort running connections in the request',
          ...
          (Request request) async {
            ...
            await Future.wait(
              [
                db.runTx(
                  (c) async {
                    await c.execute("SET application_name TO $appName");
    ...
    final response = await server.get('/dangling-conn-err?appname=$pgAppName');
    ...
    server = await runTestServer(
      extraRoutes: (app, db) {
        app.get(
          "/no-dangling",
          (Request request) async {
            final appName = await db.runTx((c) async {
              final applicationName =
                  await _getCurrentPostgresApplicationName(c);
